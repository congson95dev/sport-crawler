...
const MARCA_URL = process.env.MARCA_URL;
...
(async () => {
  ...
  try {
    // Mở trang
    await page.goto(MARCA_URL, { waitUntil: 'domcontentloaded', timeout: 60000 });
    ...
    // Load content
    await page.waitForSelector('.ue-l-cover-grid article');
    console.log('✅ Posts content đã load.');

    // Tìm tất cả post
    const posts = await page.$$('.ue-l-cover-grid article');
    console.log(`🔎 Tìm thấy ${posts.length} posts.`);

    const data = [];
    for (const post of posts) {
        const link = await post.evaluate(el => {
          const a = el.querySelector('a.ue-c-cover-content__link');
    ...
    for (const item of data) {
        ...
        try {
            ...
            const title = await page.$eval('article h1.js-headline', el => el.innerText.trim()).catch(() => '');
            console.log("title: " + title);

            const sub_title = await page.$eval('article p.ue-c-article__standfirst', el => el.innerText.trim()).catch(() => '');
            ...
            const published_date = await page.$eval('.ue-l-article__body .ue-c-article__publishdate time', el => el.innerText.trim()).catch(() => '');
