...
const SPORT_ES_URL = process.env.SPORT_ES_URL;
...
(async () => {
  ...
  try {
    // Mở trang
    await page.goto(SPORT_ES_URL, { waitUntil: 'domcontentloaded', timeout: 60000 });
    ...
    // Load content
    await page.waitForSelector('#sectionList header h2');
    console.log('✅ Posts content đã load.');

    // Tìm tất cả post
    const posts = await page.$$('#sectionList header h2');
    ...
    for (const item of data) {
        ...
        try {
            ...
            const content = await page.$$eval(
              'article > div > div > div > div > div p.ft-text',
              elements => elements.map(el => el.innerText.trim()).join('\n')
            ).catch(() => '');
            console.log("content: " + content);

            const published_date = await page.$eval('article time.ft-date__textTime', el => el.innerText.trim()).catch(() => '');
